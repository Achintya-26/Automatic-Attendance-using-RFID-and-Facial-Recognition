...
const saveAttendance = async (req, res) => {
	...
	const { student_id, branch, sem, batch } = studentRow[0];
	console.log(branch+" "+batch+" "+sem);

	const { rows: sessionRow } = await query('SELECT lecture_id, teacher_id FROM lecture_session WHERE branch = $1 AND sem= $2 AND batch= $3 AND is_active = $4', [branch,sem,batch,true]);
