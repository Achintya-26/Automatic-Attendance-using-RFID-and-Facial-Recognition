...
const createLecture = async (req, res) => {
	...
	const { branch, sem, batch, subject, start_time, end_time } = req.body;
	//! ex -> IT 7 B IOE 2023-10-18 01:40:06 2023-10-18 02:00:06
	console.log(branch, sem, batch, subject, start_time, end_time);
	const { rows: lectureRows } = await query('SELECT * FROM lecture_session WHERE branch = $1 AND sem = $2 AND batch = $3 AND is_active = $4', [branch, sem, batch, true]);
	...
	if(hasOverlapped) {
		...
		const { rows } = await query('INSERT INTO lecture_session (teacher_id, branch, sem, batch, subject, is_active, start_time, end_time) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING lecture_id', [teacher_id, branch, sem, batch, subject, true, start_time, end_time]);
